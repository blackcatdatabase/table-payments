...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 2 outbound / 5 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 2 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\payments](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\payments)
...
graph LR
  ...
  payments -->|FK| orders["orders"]:::link
  payments -->|FK| tenants["tenants"]:::accent
  idempotency_keys["idempotency_keys"]:::inbound -->|FK| payments
  payment_gateway_notifications["payment_gateway_notifications"]:::inbound -->|FK| payments
  payment_logs["payment_logs"]:::inbound -->|FK| payments
  payment_webhooks["payment_webhooks"]:::inbound -->|FK| payments
  refunds["refunds"]:::inbound -->|FK| payments
...
| Inbound links (tables depending on this) | 5 |
| Views | 12 |
| Seeds | 0 |
| Drift warnings | 0 |
| PII flags | 2 |
...
- Outbound (depends on): "orders", "tenants"
- Inbound (relies on this): "idempotency_keys", "payment_gateway_notifications", "payment_logs", "payment_webhooks", "refunds"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package payments -Apply
