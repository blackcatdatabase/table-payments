...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'tenant_id', 'order_id', 'gateway', 'transaction_id', 'provider_event_id', 'status', 'amount', 'currency', 'details', 'created_at', 'updated_at', 'version' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'gateway', 'transaction_id', 'provider_event_id', 'status', 'currency' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'tenant_id', 'order_id', 'gateway', 'transaction_id', 'provider_event_id', 'status', 'amount', 'currency', 'created_at', 'updated_at', 'version' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\Payments\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\Payments\Definitions::softDeleteColumn();
