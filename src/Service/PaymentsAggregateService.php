...
namespace BlackCat\Database\Packages\Payments\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Payments\Dto\PaymentDto;
use BlackCat\Database\Packages\Payments\Mapper\PaymentDtoMapper;
 ...
 */
final class PaymentsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PaymentRepository $paymentRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Payments\ModuleException("Record {$idStr} not found for locking.");
            }
