...
use BlackCat\Database\Packages\Payments\ContractRepository;

final class PaymentRepository implements ContractRepository {
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO payments (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO payments (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE payments SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE payments SET $set WHERE id = :id", $params);
        }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE payments SET $set WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM payments WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM payments WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM payments t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM payments t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
