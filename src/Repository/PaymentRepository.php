...
use BlackCat\Database\Packages\Payments\Repository\PaymentRepositoryInterface;
...
class PaymentRepository implements PaymentRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'transaction_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Payments\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'order_id', 'gateway', 'provider_event_id', 'status', 'amount', 'currency', 'details' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndTransactionId(int $tenantId, string $transactionId, bool $asDto = false): array|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'transaction_id' => $transactionId ], $asDto);
    }
    ...
    public function getIdByTenantIdAndTransactionId(int $tenantId, string $transactionId) {
        $row = $this->getByTenantIdAndTransactionId($tenantId, $transactionId, false);
