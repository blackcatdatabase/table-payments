...
namespace BlackCat\Database\Packages\Payments\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Payments\Definitions;
use BlackCat\Database\Packages\Payments\Criteria;
use BlackCat\Database\Packages\Payments\Dto\PaymentDto as Dto;
use BlackCat\Database\Packages\Payments\Mapper\PaymentDtoMapper as RowMapper;
...
final class PaymentRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Payments\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'transaction_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Payments\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'transaction_id' ], [ 'order_id', 'gateway', 'provider_event_id', 'status', 'amount', 'currency', 'details' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null */
    public function getByTenantIdAndTransactionId(int $tenantId, string $transactionId, bool $asDto = false): array|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'transaction_id' => $transactionId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Payments\Mapper\PaymentDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndTransactionId(int $tenantId, string $transactionId): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'transaction_id') . ' = :uniq_transaction_id';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_transaction_id' => $transactionId ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndTransactionId(int $tenantId, string $transactionId) {
        $row = $this->getByTenantIdAndTransactionId($tenantId, $transactionId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Payments\Dto\PaymentDto|null {
